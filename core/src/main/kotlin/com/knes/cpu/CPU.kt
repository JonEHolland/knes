...
class CPU(val bus: Bus) {
    ...
    data class I(
        ...
        fun pretty(pc: Int, cpu: CPU): String {
    ...
    fun tick() {
        with(bus.state.cpu) {
            ...
            if (cyclesRemaining <= 0) {
                ...
                if (log) {
                    println(currentInstruction!!.pretty(pcTmp, this@CPU) + "   " + busPrev)
                }
    ...
    fun setFlag(flag: Flags, value: Boolean) {
        with(bus.state.cpu) {
            status = if (value) {
                (status or flag.value)
            } else {
                (status and flag.value.inv())
            }
        }
    }
    ...
    private fun interrupt(cyclesNeeded: Int, vectorAddress: Int) {
    ...
    fun decompile(): String? {
        with(bus.state.cpu) {
            ...
            return currentInstruction!!.pretty(address, this@CPU)
        }
    }
